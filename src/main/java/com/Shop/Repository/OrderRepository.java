...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query("SELECT SUM(oi.profit) FROM Order o " + "JOIN o.orderItems oi "
	...
	@Query(value = "SELECT SUM(oi.profit) FROM \"user_order\" o "
	...
	@Query(value = "SELECT SUM(oi.profit) FROM \"user_order\" o "
			+ "JOIN \"user_order_order_items\" rl ON o.id = rl.order_id "
			+ "JOIN order_item oi ON rl.order_items_id = oi.id "
			+ "WHERE EXTRACT(YEAR FROM o.order_date) = :year", nativeQuery = true)
	...
	// -------------------------------------
	@Query(value = "SELECT p.name as productName, SUM(oi.quantity) as soldQuantity , (p.amount - SUM(oi.quantity)) as remainingQuantity "
